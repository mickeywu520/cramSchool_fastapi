...
async def get_homepage_data(db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "announcements": [
            {"id": a.id, "title": a.title, "content": a.content[:200], "priority": a.priority,
             "published_at": a.published_at}
            for a in announcements
        ],
        "featured_teachers": [
            {"id": t.id, "name": t.name, "subject": t.subject, "title": t.title,
             "motto": t.motto, "photo_url": t.photo_url}
            for t in teachers
        ],
        "latest_honors": [
            {"id": h.id, "student_name": h.student_name, "school": h.school,
             "department": h.department, "year": h.year}
            for h in honors
        ],
        "early_bird_courses": [
            {"id": c.id, "name": c.name, "subject": c.subject, "category": c.category,
             "price": c.price, "early_bird_discount": c.early_bird_discount}
